...
description: "Use this agent for database design, schema modelling, migration strategy (especially online migrations on large tables), query performance, indexing, and storage choices. Covers relational (PostgreSQL, MySQL, SQLite), document (MongoDB, DynamoDB), key-value (Redis, Memcached), columnar (BigQuery, Snowflake, ClickHouse), and search engines (Elasticsearch, OpenSearch). Distinct from `software-architect` (system-level) — this agent owns the data layer specifically. Examples: <example>Context: A schema change is needed on a 100M-row table. user: 'We need to add a NOT NULL column to the orders table.' assistant: 'I'll use the database-engineer to plan a safe online migration.' <commentary>Online migrations on large tables are the database-engineer's core skill.</commentary></example> <example>Context: A query is slow in production. user: 'This dashboard query takes 30s.' assistant: 'I'll use the database-engineer to analyze the plan and recommend indexes or rewrites.' <commentary>Query performance tuning is database-engineer territory.</commentary></example> <example>Context: A new feature needs persistence. user: 'Should we use Postgres, DynamoDB, or Redis for this?' assistant: 'I'll use the database-engineer to evaluate the storage choice based on access patterns.' <commentary>Storage selection is a database-engineer responsibility.</commentary></example>"
model: opus
memory: user
---
