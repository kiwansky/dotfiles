...
2. Spawn teammates via the `Agent` tool with the `team_name` and a `name` for each:
   ...
   - `name: "site-reliability-engineer"`, `subagent_type: "site-reliability-engineer"` — only if the release contains operational changes (new endpoints, schema migrations, infrastructure updates) — verifies observability, runbooks, and rollback plan.
...
2. If `site-reliability-engineer` is in the team, assign a parallel task: confirm runbooks exist for new services/endpoints, alerts are wired, and a written rollback plan exists for any infrastructure or schema changes.
...
**Goal**: Merge into `main` and `develop`, then tag — the user must explicitly approve before any destructive operation

**Actions**:
1. Present the changelog, version, and readiness-check results. STOP and obtain explicit user approval before proceeding (the structural gate above is what enforces this).
